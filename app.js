...
/* ==========================================================
   Seguridad Primavera – JavaScript principal  v 4.3
   ========================================================== */
...
/* ── Inicialización: Desactivar todos los guardias al cargar ── */
(function initGuards() {
  if (!ls.get('activeTurn')) {
...
(function ensureBaseUsers() {
  ...
  if (iSuper === -1) {
    ...
    users[iSuper] = { ...users[iSuper], ...SUPER };
  }
...
/* ═════════════════ Funciones de turno ════════════════════ */
const setActiveTurn = (active) => {
  if (active) {
    // Verificar si hay otro guardia activo
    const currentTurn = ls.get('activeTurn');
    if (currentTurn && currentTurn.user !== currentUser) {
      const activeGuardUser = ls.get('users').find(user => user.user === currentTurn.user);
      const activeGuardDetails = ls.get('guards').find(g => g.id === activeGuardUser?.guardId);
      const guardName = activeGuardDetails?.name || "Un guardia (ID: " + (activeGuardUser?.guardId || '?') + ")";
      
      Swal.fire({
        title: 'Turno ocupado',
        html: `El guardia <b>${guardName}</b> ya está activo.<br>No puedes activar tu turno mientras otro guardia esté en servicio.`,
        icon: 'warning'
      });
      return false;
    }
...
const updateTurnUI = () => {
  const toggleBtn = $('#toggleTurnBtn');
  const statusSpan = $('#turnStatus');
  ...
  if (isTurnActive) {
    toggleBtn.textContent = 'Desactivar turno';
    toggleBtn.classList.remove('btn-amber');
    toggleBtn.classList.add('btn-red');
    statusSpan.textContent = 'Turno activo';
    statusSpan.className = 'ml-2 px-2 py-1 rounded text-xs bg-green-500';
  } else {
    toggleBtn.textContent = 'Activar turno';
    toggleBtn.classList.remove('btn-red');
    toggleBtn.classList.add('btn-amber');
    statusSpan.textContent = 'Turno inactivo';
    statusSpan.className = 'ml-2 px-2 py-1 rounded text-xs bg-gray-500';
  }
};

/* ═════════════════ Funciones para gestión de guardias ═════════════════ */
const desactivarTodosLosGuardias = () => {
  ls.set('activeTurn', null);
  isTurnActive = false;
  if (role === 'guard') updateTurnUI();
  return { success: true, message: 'Todos los turnos de guardia han sido desactivados' };
};

const desactivarGuardiaPorId = (guardiaId) => {
  const activeTurn = ls.get('activeTurn');
  if (activeTurn && activeTurn.guardId === guardiaId) {
    ls.set('activeTurn', null);
    if (currentGuard?.id === guardiaId) {
      isTurnActive = false;
      updateTurnUI();
    }
    return { success: true, message: `Turno del guardia ID ${guardiaId} desactivado` };
  }
  return { success: false, message: `El guardia ID ${guardiaId} no tenía turno activo` };
};
